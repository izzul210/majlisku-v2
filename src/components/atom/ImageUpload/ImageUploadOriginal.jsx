...
function ImageUpload({ defaultImgUrl = null, dispatch = () => {}, type = 'SET_IMAGE' }) {
	...
	const [originalFileForCroppedImg, setOriginalFileForCroppedImg] = useState(null);
	...
	const handleCropImage = () => {
		setImageUrl(originalImgUrl);
		setEditImageModal(false);
		setCropImageModal(true);
		initCompleteCrop(imgPreviewRef?.current.width, imgPreviewRef.current.height);
	};
	...
	function onCropSave() {
		...
		previewCanvasRef.current.toBlob((blob) => {
			...
			setOriginalFileForCroppedImg(file);
			...
			<Button component='label' sx={{ p: 0 }} className='w-full bg-black'>
				<div
					className={`flex py-2 rounded-lg ${
						imageUrl ? '' : 'border border-gray-400 border-dashed'
					} justify-center  items-center w-full h-auto`}
					style={{ backgroundColor: imageUrl ? 'white' : '#FCFCFD' }}>
					{imageUrl ? (
				...
				{!imageUrl && !originalImgUrl ? (
					...
					<input hidden onClick={() => setEditImageModal(true)} id='click' />
				)}
			</Button>
			...
			<ModalProvider2
				...
				<div className='flex flex-col p-4 gap-2'>
					<Button component='label' sx={{ p: 0 }}>
						<div className='border w-full flex py-2 rounded-lg justify-center items-center border-gray-400 border-dashed'>
							<div className='flex flex-col gap-0 p-3 justify-center items-center normal-case'>
								<div className='my-2'>
									<ImageUploadIcon />
								</div>
